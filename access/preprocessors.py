...
from .utils.cal import (get_lexical_complexity_score, get_levenshtein_similarity,
                                       get_dependency_tree_depth)
from .utils.paths import MODELS_DIR, get_data_filepath
from .utils.utils import (write_lines_in_parallel, yield_lines_in_parallel, add_dicts, get_default_args,
...
class SentencePiecePreprocessor(AbstractPreprocessor):
    ...
    def __init__(self, vocab_size=10000, input_filepaths=None):
        ...
        self.sentencepiece_model_path = MODELS_DIR / f'sentencepiece-model/sentencepiece_model_{self.vocab_size}.model'
        self.input_filepaths = input_filepaths
        if self.input_filepaths is None:
            self.input_filepaths = [
                get_data_filepath('wikilarge', 'train', 'src'),
                get_data_filepath('wikilarge', 'train', 'dst')
            ]
