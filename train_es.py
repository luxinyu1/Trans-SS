...
def sari_validate(cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr, subsets: List[str]) -> List[Optional[float]]:
    ...
    complex_filepath = get_data_filepath('simplext', 'valid', 'complex')
    ...
    ref_filepaths = [get_data_filepath('simplext', 'valid', 'simple')]
    simplext_scores = get_all_scores(orig_sents=read_lines(complex_filepath), sys_sents=read_lines(pred_filepath), refs_sents=[read_lines(ref_filepath) for ref_filepath in ref_filepaths])
    simplext_sari = simplext_scores["SARI"]
    logger.info("[Epoch {}][simplext] {}".format(epoch_itr.epoch, simplext_scores))
    with open(log_filepath, 'a+', encoding='utf-8') as f_log:
        f_log.write("[Epoch {}][simplext] {}\n".format(epoch_itr.epoch, simplext_scores))
    if not hasattr(trainer, 'best_sari'):
        trainer.best_sari = 0
    if not hasattr(trainer, 'n_validations_since_best'):
        trainer.n_validations_since_best = 0
    if simplext_sari > trainer.best_sari:
        trainer.best_sari = simplext_sari
        trainer.n_validations_since_best = 0
    else:
