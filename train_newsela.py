...
def sari_validate(cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr, subsets: List[str]) -> List[Optional[float]]:
    ...
    complex_filepath = get_data_filepath('newsela', 'valid', 'src')
    os.system("cat {} > complex.txt".format(complex_filepath))
    ...
    ref_filepath = get_data_filepath("newsela", "valid", "dst")
    os.system("cat {} > pred.txt".format(pred_filepath))
    os.system("cat {} > ref.txt".format(ref_filepath))
    newsela_scores = get_all_scores(orig_sents=read_lines(complex_filepath), sys_sents=read_lines(pred_filepath), refs_sents=[read_lines(ref_filepath)])
    newsela_sari = newsela_scores["SARI"]
    logger.info("[Epoch {}][newsela] {}".format(epoch_itr.epoch, newsela_scores))
    with open(log_filepath, 'a+', encoding='utf-8') as f_log:
        f_log.write("[Epoch {}][newsela] {}\n".format(epoch_itr.epoch, newsela_scores))
    if not hasattr(trainer, 'best_sari'):
        trainer.best_sari = 0
    if not hasattr(trainer, 'n_validations_since_best'):
        trainer.n_validations_since_best = 0
    if newsela_sari > trainer.best_sari:
        trainer.best_sari = newsela_sari
        trainer.n_validations_since_best = 0
    else:
