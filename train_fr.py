...
def sari_validate(cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr, subsets: List[str]) -> List[Optional[float]]:
    ...
    complex_filepath = get_data_filepath('alector', 'valid', 'complex')
    ...
    ref_filepaths = [get_data_filepath('alector', 'valid', 'simple')]
    alector_scores = get_all_scores(orig_sents=read_lines(complex_filepath), sys_sents=read_lines(pred_filepath), refs_sents=[read_lines(ref_filepath) for ref_filepath in ref_filepaths])
    alector_sari = alector_scores["SARI"]
    logger.info("[Epoch {}][alector] {}".format(epoch_itr.epoch, alector_scores))
    with open(log_filepath, 'a+', encoding='utf-8') as f_log:
        f_log.write("[Epoch {}][alector] {}\n".format(epoch_itr.epoch, alector_scores))
    if not hasattr(trainer, 'best_sari'):
        trainer.best_sari = 0
    if not hasattr(trainer, 'n_validations_since_best'):
        trainer.n_validations_since_best = 0
    if alector_sari > trainer.best_sari:
        trainer.best_sari = alector_sari
        trainer.n_validations_since_best = 0
    else:
