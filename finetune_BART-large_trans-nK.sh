if [ ! -d "./datasets/trans-1K/" ]; then
    python ./split.py --use-num 1000 \
        --output-dir './datasets/trans-1K/' \
        --dataset 'trans'
fi
...
if [ "$1" != "no-preprocess" ]; then
    ...
    for split in 'train' 'test' 'valid'; do
        for type in 'src' 'dst'; do
            ...
            --inputs ./datasets/trans-1K/trans.${split}.${type} \
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python ./train.py ${TASK}-bin/ \
    ...
    --tensorboard-logdir "./logs/tensorboard/trans-1K/bart-large-pretrained/" \
    --save-dir "./checkpoints/trans-1K/bart-large-pretrained/" \
